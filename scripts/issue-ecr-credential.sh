...
### Collect ECR holder information

echo ""
echo "Input ECR Holder information"
read -p "ECR Holder LEI: " -r ecr_lei
read -p "ECR Holder OOBI: " -r ecr_oobi
read -p "ECR Holder Legal Name: " -r ecr_legal_name
read -p "ECR Holder Role: " -r ecr_role
echo ""
echo "Now resolve OOBI of the Legal Entity Issuer before proceeding"
echo "$acme_oobi"
echo ""
...
ecr_aid=$(curl -s -D - -o /dev/null "$ecr_oobi" | grep -i Keri-Aid | cut -d ' ' -f 2 | tr -d '\r')
...
## Create Engagement Context Role credential for ECR

kli oobi resolve --name "$acme_name" --oobi "$ecr_oobi"
...
kli vc create --name "$acme_name" --alias acme --registry-name vlei --schema "$schema_ecr_said" --recipient "$ecr_aid" --private \
  --data "{\"LEI\": \"$ecr_lei\", \"engagementContextRole\":\"$ecr_role\", \"personLegalName\": \"$ecr_legal_name\"}" \
...
kli ipex grant --name "$acme_name" --alias acme --recipient "$ecr_aid" --said "$acme_conductor_credential_said"
